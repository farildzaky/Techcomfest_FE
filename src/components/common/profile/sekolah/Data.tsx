...
const CustomDropdown = ({ label, options, isOpen, onToggle, onSelect }: any) => (
    <div className="relative w-full">
        <button type="button" onClick={onToggle} className={`w-full cursor-pointer text-white satoshiBold text-[1.2vw] py-[0.8vw] px-[1.5vw] rounded-[0.5vw] flex justify-between items-center shadow-sm transition-all border border-transparent ${isOpen ? 'bg-[#b06a33]' : 'bg-[#E87E2F] hover:bg-[#d6732a]'} shadow-[0px_4px_4px_rgba(0,0,0,0.25)]`}>
            <span className="truncate mr-[0.5vw]">{label}</span>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className={`w-[1.2vw] h-[1.2vw] min-w-[1.2vw] transition-transform ${isOpen ? 'rotate-180' : ''}`}><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
        </button>
        {isOpen && (
            <div className="absolute top-[120%] left-0 w-full bg-white rounded-[0.5vw] shadow-xl max-h-[15vw] overflow-y-auto border border-orange-200 z-[100] animate-in fade-in zoom-in-95 duration-100 custom-scrollbar">
                {options.map((opt: any, idx: number) => (
                    <div key={idx} onMouseDown={() => onSelect(opt)} className="px-[1.5vw] py-[0.8vw] hover:bg-orange-50 cursor-pointer satoshiMedium text-[1vw] text-gray-700 border-b border-gray-100 last:border-0 hover:text-[#E87E2F] transition-colors">{opt}</div>
                ))}
            </div>
        )}
    </div>
    ...
    return (
        <div className="w-full min-h-screen py-[2vw] px-[3vw] flex flex-col font-sans relative bg-[#E87E2F]"> 
            <div className="mb-[2vw]">
                <h1 className='satoshiBold text-[3vw] text-white'>Rekap Pelaporan</h1>
            ...
            <div ref={filterContainerRef} className="flex flex-col items-start relative z-50 mb-[2vw]">
                ...
                {isFilterActive && (
                    <button onClick={handleClearFilter} className="bg-white w-fit text-[#E87E2F] cursor-pointer satoshiMedium text-[1.2vw] py-[0.8vw] px-[1.5vw] rounded-[1vw] flex items-center gap-[0.5vw] shadow-sm hover:bg-[#E87E2F] hover:text-white transition-all shadow-[0px_4px_4px_0px_#00000040]">
            ...
            {!loading && !error && filteredReports.length > 0 && (
                <div className="flex justify-center items-center gap-[2vw] mt-[2vw]">
                    ...
                    <span className="text-white satoshiBold text-[1.2vw] drop-shadow-md">Halaman {currentPage} dari {totalPages}</span>
                    <button onClick={() => handleNext(totalPages)} disabled={currentPage === totalPages} className={`px-[2vw] py-[0.6vw] rounded-full text-[1.2vw] satoshiBold ${currentPage === totalPages ? 'bg-black/20 text-white/50 cursor-not-allowed' : 'bg-[#FFF3EB] text-[#E87E2F] hover:scale-105 shadow-md'}`}>Selanjutnya</button>
                </div>
            )}
            ...
            {selectedDetail && (
                <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-[3vw]">
                    <div className="bg-[#E87E2F] w-[50vw] max-h-[90vh] overflow-y-auto rounded-[2vw] p-[3vw] shadow-2xl relative flex flex-col gap-[1.5vw] animate-in fade-in zoom-in-95 duration-200">
                        {loadingDetail ? <div className="text-white text-center satoshiBold text-[1.5vw]">Mengambil Data Detail...</div> : (
                            <>
                                ...
                                <div className="bg-white rounded-[1.5vw] p-[2vw] min-h-[10vw]">
                                    <ul className="list-none flex flex-col gap-[0.8vw]">
                                        {selectedDetail.catatan.map((line, idx) => <li key={idx} className="text-[#8B4513] satoshiMedium text-[1.2vw] leading-relaxed">{line}</li>)}
                                    </ul>
                                </div>
                            </>
                        )}
                        <button onClick={() => setSelectedDetail(null)} className="absolute top-[1.5vw] right-[2vw] text-white hover:text-gray-200 cursor-pointer transition-transform hover:scale-110">
