...
const DaftarSppgAdmin = () => {
    ...
    useEffect(() => {
        const fetchSppgData = async () => {
            ...
            try {
                ...
                if (!res.ok) throw new Error("Gagal mengambil data SPPG");

                const responseData = await res.json();
                
                if (responseData && responseData.data) {
                    const sppgUsers = responseData.data.filter((user: UserData) => user.role === 'sppg');
                    setSppgList(sppgUsers);
                }
    ...
    const handleStatusChange = async (id: string, newStatus: string) => {
        const originalList = [...sppgList];
        const normalizedStatus = newStatus.toLowerCase();

        setSppgList(prev => prev.map(item => 
        ...
        try {
            ...
            setSppgList(originalList);
        }
    };

    const filteredSppg = sppgList.filter(s => 
        filterStatus === "All" || s.status.toLowerCase() === filterStatus.toLowerCase()
    );

    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    const currentItems = filteredSppg.slice(indexOfFirstItem, indexOfLastItem);
    const totalPages = Math.ceil(filteredSppg.length / itemsPerPage);
    ...
    if (error) return (
        <div className="w-full h-screen flex flex-col justify-center items-center gap-4 lg:gap-[1vw]">
            <p className="text-red-500 satoshiBold text-xl lg:text-[1.5vw]">{error}</p>
            <button onClick={() => window.location.reload()} className="bg-[#E87E2F] text-white px-4 py-2 lg:px-[2vw] lg:py-[0.5vw] rounded-lg lg:rounded-[0.5vw]">Muat Ulang</button>
        </div>
    );

    return (
        // Container Utama: Padding standar di mobile (p-4), VW di desktop (p-[3vw])
        <div className="w-full min-h-screen p-4 lg:p-[3vw] font-sans relative" onClick={() => { setOpenRowId(null); setIsFilterOpen(false); }}>
            ...
            <div className="flex justify-between items-center mb-6 lg:mb-[2vw]">
                <h1 className="satoshiBold text-2xl lg:text-[2.5vw] text-black">SPPG</h1>
            ...
            <div className="w-full bg-[#E87E2F] rounded-xl lg:rounded-[1.5vw] border-2 lg:border-[0.2vw] border-[#E87E2F] overflow-x-auto">
                ...
                <div className="min-w-[900px] w-full">
                    ...
                    <div className="flex bg-[#E87E2F] text-white rounded-t-lg lg:rounded-t-[1.3vw]">
                        {/* Kolom Header: Mobile (text-base py-4), Desktop (text-[1.5vw] py-[1vw]) */}
                        ...
                        <div className="w-[30%] py-4 lg:py-[1vw] flex justify-center items-center border-r border-white lg:border-r-[0.15vw] satoshiBold text-base lg:text-[1.5vw]">SPPG</div>
                        <div className="w-[25%] py-4 lg:py-[1vw] flex justify-center items-center border-r border-white lg:border-r-[0.15vw] satoshiBold text-base lg:text-[1.5vw]">Jumlah Sekolah</div>
                    ...
                    <div className="flex flex-col bg-white lg:rounded-b-[1.3vw]">
                        {loading ? (
                            Array.from({ length: itemsPerPage }).map((_, index) => (
                                <div key={index} className="flex border-b border-[#E87E2F] lg:border-b-[0.15vw] animate-pulse bg-white">
                                    ...
                                    <div className="w-[20%] py-4 lg:py-[1.5vw] flex justify-center items-center border-r border-[#D9833E] lg:border-r-[0.15vw]">
                                        <div className="h-4 w-12 lg:h-[1.5vw] lg:w-[6vw] bg-gray-200 rounded"></div>
                                    </div>
                            ...
                            currentItems.map((item, index, array) => {
                                ...
                                const displayStatus = item.status ? item.status.charAt(0).toUpperCase() + item.status.slice(1) : "Unknown";
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`flex border-b border-[#E87E2F] lg:border-b-[0.15vw] transition-colors relative z-10 
                                            ...
                                            ${index === array.length - 1 ? 'last:border-b-0 lg:rounded-b-[1.3vw]' : ''} 
                                        `}
                                    ...
                                    >
                                        ...
                                        <div className="w-[15%] py-4 lg:py-[1.5vw] flex justify-center items-center">
                                            <Link href={`/admin/sppg/${item.id}`} className="text-[#D9833E] underline satoshiMedium text-sm lg:text-[1.2vw] hover:text-[#b06a33]">
                                                Lihat Detail
                                            </Link>
                                        </div>
                                    </div>
            ...
            <div className="flex justify-end mt-4 lg:mt-[1.5vw] items-center gap-4 lg:gap-[1vw]">
                <button 
                    ...
                    className={`w-8 h-8 lg:w-[2vw] lg:h-[2vw] flex items-center justify-center rounded-full border border-[#E87E2F] transition-colors ${currentPage === 1 ? 'bg-gray-200 text-gray-400 border-gray-300 cursor-not-allowed' : 'bg-white text-[#E87E2F] hover:bg-[#E87E2F] hover:text-white'}`}
                >
                    &lt;
                </button>
                ...
                <button 
                    ...
                    className={`w-8 h-8 lg:w-[2vw] lg:h-[2vw] flex items-center justify-center rounded-full border border-[#E87E2F] transition-colors ${currentPage === totalPages ? 'bg-gray-200 text-gray-400 border-gray-300 cursor-not-allowed' : 'bg-white text-[#E87E2F] hover:bg-[#E87E2F] hover:text-white'}`}
                >
                    &gt;
                </button>
            </div>
...
export default DaftarSppgAdmin;